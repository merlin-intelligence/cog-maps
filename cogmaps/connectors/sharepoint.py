    ...
    def download_folder(self, folder_url: str, destination: str, log_callback=None) -> None:
        ...
        for file in folder.files:
            if not any(file.name.lower().endswith(ext) for ext in SUPPORTED_EXTENSIONS):
                ...
                continue
            safe_name = sanitize_remote_name(file.name)
            file_path = os.path.join(destination, safe_name)
            if log_callback:
        ...
        for sub in folder.folders:
            if sub.name not in SKIP_FOLDERS:
                safe_sub_name = sanitize_remote_name(sub.name)
                self.download_folder(
                    sub.server_relative_url, os.path.join(destination, safe_sub_name), log_callback
                )
