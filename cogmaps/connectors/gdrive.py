...
from cogmaps.config import CHUNKNORRIS_EXTENSIONS
...
class GDriveClient:
    ...
    def download_folder(self, folder_id: str, destination: str, log_callback=None) -> None:
        ...
        # A real Drive file/folder ID never contains a quote — reject anything
        # else outright rather than interpolating it into the query string
        # below, where a quote would let it escape the `'...' in parents` clause.
        if not _DRIVE_ID_RE.match(folder_id):
            raise ValueError(f"Invalid Google Drive folder ID: {folder_id!r}")
        ...
        for item in items:
            name = sanitize_remote_name(item["name"])
