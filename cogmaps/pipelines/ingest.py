...
from cogmaps.core.chunking import chunk_with_chunknorris
from cogmaps.core.embeddings import EmbeddingModel, detect_device
from cogmaps.qdrant.store import QdrantStore, make_point
...
def _walk_and_classify(
    ...
    """Single-pass directory walk: report unsupported/skipped files inline, return the rest.

    Replaces two separate `os.walk` traversals (one just to report recognised-but-
    unsupported formats, one to collect the supported files) with one — halves the
    directory I/O on large or network-mounted corpora. Files matching neither
    extension tuple are silently ignored, as before.
    """
    out: list[str] = []
    skipped = 0
        ...
        for root, _, files in os.walk(real):
            for fn in files:
                ...
                if lower.endswith(unsupported_extensions):
                    ...
                    log(f"Skipping {fn}: format '{ext}' is not supported for now.")
                    continue
...
class Ingester:
    ...
    def __init__(
        ...
        self.logs: list[str] = []
        # Populated by _chunknorris_points; used by callers to tell "no new files
        # to index" (both 0) apart from "every attempted file errored out".
    ...
    def run_chunknorris(self, directories_file_path: str, collection: str) -> list[str]:
        ...
        # Single pass: reports recognised-but-unsupported formats inline and
        # returns the supported files (see _walk_and_classify's docstring).
        files = _walk_and_classify(directories, CHUNKNORRIS_EXTENSIONS, UNSUPPORTED_EXTENSIONS, skip, log)
        if not files:
    ...
    def _chunknorris_points(self, files: list[str], embedder: EmbeddingModel, log):
        self.files_attempted = len(files)
