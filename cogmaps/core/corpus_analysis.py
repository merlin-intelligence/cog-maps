...
def to_dataframe(docs: list[CorpusDocument]) -> pd.DataFrame:
    ...
    """
    # dtype="object" is required even for an empty list — otherwise pandas infers
    # float64 for an empty column, and `.str.len()` below raises AttributeError.
    df = pd.DataFrame({
        "title": pd.array([d.title for d in docs], dtype="object"),
        "content": pd.array(
            [unicodedata.normalize("NFC", d.content or "") for d in docs], dtype="object"
        ),
        "type": pd.array([d.doc_type for d in docs], dtype="object"),
    })
...
def _stack_vectors(docs: list[CorpusDocument]) -> np.ndarray | None:
    """Return an ``(n_docs, dim)`` matrix of L2-normalised vectors for ``docs``, or ``None``.

    Callers are expected to have already filtered out documents with no vector
    (see ``d.vector is not None``) — this only guards against an empty list.
    """
...
def topic_model(
    ...
    *,
    ...
    Documents with no usable vector are excluded rather than disabling the whole
    analysis; ``df`` must be row-aligned with ``docs`` (as built by
    :func:`to_dataframe` from the same list). Returns ``None`` when there aren't
    enough documents with a usable vector to form a valid clustering — sklearn's
    ``silhouette_score`` requires ``2 <= n_clusters <= n_samples - 1``, so at
    least ``k_min + 1`` documents are needed (not just ``k_min``, which would
    let ``n_clusters == n_samples`` slip through and raise a ``ValueError``).
    """
    vector_mask = np.array([d.vector is not None for d in docs])
    if int(vector_mask.sum()) < k_min + 1:
        return None
    vectors = _stack_vectors([d for d in docs if d.vector is not None])
    df = df.loc[vector_mask].reset_index(drop=True)

    n_docs = len(vectors)
    effective_kmax = min(k_max, n_docs - 1)
...
def similar_pairs(
    ...
    *,
    ...
    Pairs are sorted by descending score. Documents with no vector are excluded
    rather than disqualifying the whole corpus.
    """
    usable = [d for d in docs if d.vector is not None]
    if len(usable) < 2:
        return []
    vectors = _stack_vectors(usable)
    titles = [d.title for d in usable]
    sim = vectors @ vectors.T
    # Upper triangle only (i < j), vectorized rather than a Python double loop —
    # matches the pattern already used in cogmaps.graph.explorer. The explicit
    # boolean mask (rather than zeroing the lower triangle then comparing to
    # threshold) stays correct even for threshold <= 0.
    mask = np.triu(np.ones(sim.shape, dtype=bool), k=1) & (sim >= threshold)
    rows, cols = np.where(mask)
    pairs = [
        SimilarityPair(a=titles[i], b=titles[j], score=float(sim[i, j]))
