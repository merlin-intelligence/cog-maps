...
class QdrantStore:
    ...
    def is_reachable(cls, host: str | None = None, port: int | None = None) -> bool:
        ...
        try:
            QdrantClient(
                host=host or qdrant_host(), port=port or qdrant_port(),
                timeout=3, api_key=qdrant_api_key(), https=False,
            ).get_collections()
            return True
        except Exception:
            logger.debug("Qdrant unreachable at %s:%s", host or qdrant_host(), port or qdrant_port(), exc_info=True)
            return False
    ...
    def collection_stats(self, name: str) -> tuple[int | None, int | str | None]:
        ...
        try:
            ...
            logger.warning("Failed to fetch stats for collection %r", name, exc_info=True)
            return None, None
    ...
    def all_documents(
        ...
        Returns ``(documents, total_document_count)``. ``documents`` maps
        ``{filename: {"text": str, "vector": np.ndarray | None}}`` where ``text``
        is the chunks concatenated in ``chunk_number`` order and ``vector`` is the
        L2-normalised mean of the chunk embeddings (``None`` if a file has no
        usable vectors stored). ``total_document_count`` is always the true
        number of distinct documents in the collection.

        If the collection holds more than ``max_documents`` distinct files, only
        a deterministic random sample of that size is loaded (full text +
        vectors for every chunk of every document) — doing this for a very large
        corpus can exhaust memory on the shared Streamlit process. Compare
        ``len(documents)`` to ``total_document_count`` to tell a sample apart
        from the full collection. Pass ``max_documents=None`` to always load
        everything.
        """
        all_filenames = self.existing_filenames(collection)
        total = len(all_filenames)

        scroll_filter = None
        if max_documents is not None and total > max_documents:
            sample = random.Random(sample_seed).sample(sorted(all_filenames), max_documents)
            scroll_filter = models.Filter(
                must=[models.FieldCondition(key="filename", match=models.MatchAny(any=sample))]
            )
