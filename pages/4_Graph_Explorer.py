                        ...
                        for i, ch in enumerate(art.ranked_chunks):
                            ...
                            with st.expander(label):
                                st.markdown(f'<p style="margin-bottom:0.5rem">{badges_html}</p>',
                                            unsafe_allow_html=True)
                                if ch.get("themes"):
                                    # themes are extracted from corpus text (spaCy nouns) — escape
                                    # for consistency with the other corpus-derived HTML on this page.
                                    safe_themes = html.escape(", ".join(ch["themes"]))
                                    st.markdown(
                                        f'<p style="font-family:\'DM Mono\',monospace;'
                                        f'font-size:0.72rem;color:#8a6a50">'
                                        f'themes: {safe_themes}</p>',
                                        unsafe_allow_html=True,
                                    )
                                st.write(ch["text"])
