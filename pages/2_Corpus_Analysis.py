...
from cogmaps.config import DOCUMENT_COLORS, max_analysis_documents
...
with st.spinner("reconstructing documents from chunks…"):
    records, total_documents = store.all_documents(qdrant_col, max_documents=max_analysis_documents())

if not records:
    empty_state("💨", "This collection is empty.")
    st.stop()

if len(records) < total_documents:
    st.warning(
        f"This collection has {total_documents:,} documents — to keep memory usage "
        f"bounded, this analysis is based on a random sample of {len(records):,}. "
        "Raise the `MAX_ANALYSIS_DOCUMENTS` env var to analyze more at once."
    )
...
if model is None:
    # topic_model() needs at least k_min + 1 usable documents (its default
    # k_min is 2) — silhouette scoring requires fewer clusters than samples,
    # so exactly 2 documents can never form a valid clustering.
    st.info("Not enough documents with embeddings to cluster (need at least 3).")
else:
    st.markdown(
    ...
    for i in range(model.best_k):
        ...
        color = PALETTE[i % len(PALETTE)]
        # cluster_label is extracted from corpus text (TF-IDF top terms) — escape
        # before embedding in raw HTML to avoid stored XSS via crafted document text.
        header = (
            f'<span style="display:inline-block;width:10px;height:10px;border-radius:50%;'
            f'background:{color};margin-right:8px;vertical-align:middle"></span>'
            f'<strong>Cluster #{i + 1}</strong> · '
            f'<span style="color:#8a6a50">{html.escape(model.cluster_label[i])}</span> · '
...
if not pairs:
    ...
    for p in pairs:
        ...
        # p.a/p.b are document titles (derived from uploaded filenames) — escape
        # before embedding in raw HTML to avoid stored XSS via a crafted filename.
        safe_a, safe_b = html.escape(p.a), html.escape(p.b)
        st.markdown(
            f"""
