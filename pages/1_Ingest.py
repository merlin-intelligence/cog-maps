...
from cogmaps.config import ocr_available, sharepoint_credentials
...
try:
    from cogmaps.connectors.sharepoint import SharePointClient
...
def _safe_path_component(name: str) -> str:
    """Reduce a value to something safe to embed as a filesystem path segment.

    ``collection_name`` (typed by the user, or read back from an existing
    Qdrant collection) is never a full path, but a name containing ``/`` or
    ``..`` would otherwise let ``os.path.join``/f-string interpolation escape
    the intended user/collection directory.
    """
...
history_file = os.path.join(user_dir, f"history_{_safe_path_component(qdrant_col_name)}.txt")
...
def _remote_download_and_ingest(downloader: Callable[[str], None]) -> None:
    ...
    download_dir = os.path.join(user_corpus, _safe_path_component(collection_name))
...
if ingestion_source == "Local Directories":
    ...
    _local_mode_options = ["Upload files"]
    if is_admin():
        # "Paths file" and "Folder path" both let the caller point at arbitrary
        # server-side directories — restricted to admins since a non-admin
        # could otherwise ingest (and later read back via chat/analysis) any
        # file readable by the app process, e.g. another user's data.
        _local_mode_options = ["Paths file (.txt)", *_local_mode_options, "Folder path"]
    local_mode = st.radio(
        "input mode",
        _local_mode_options,
        horizontal=True,
    ...
    if local_mode == "Paths file (.txt)":
        if not is_admin():
            st.error("Paths-file ingestion is restricted to administrators.")
            st.stop()
        info_box(
        ...
        if st.button("▶ start embedding", type="primary"):
            if uploaded_files and collection_name:
                upload_dir = os.path.join(
                    "user_data", current_user(), "uploads", _safe_path_component(collection_name)
                )
                os.makedirs(upload_dir, exist_ok=True)
                try:
                    for uf in uploaded_files:
                        safe_name = _safe_upload_filename(uf.name)
        ...
        if not is_admin():
            st.error("Folder path ingestion is restricted to administrators.")
            st.stop()
        info_box(
        ...
        if st.button("▶ download & embed", type="primary"):
            ...
            if gdrive_folder_id and secrets_file and collection_name:
                ...
                try:
                    ...
                    st.error(f"Error: {e}")
                    logger.exception("Google Drive ingestion failed")
                finally:
...
# ══ SHAREPOINT ═════════════════════════════════════════════════════════
    ...
    if not SHAREPOINT_AVAILABLE:
        ...
        # The org-wide default secret must never be written into the page's HTML
        # for non-admins — a `value=` on a password input is still readable via
        # devtools. Non-admins see blank fields and the server falls back to the
        # env default at submit time if they leave them empty.
        col_a, col_b = st.columns(2)
        with col_a:
            client_id = st.text_input(
                "Client ID", value=default_id if is_admin() else "",
                placeholder=None if is_admin() else "(using organization default)",
            )
        with col_b:
            client_secret = st.text_input(
                "Client Secret", value=default_secret if is_admin() else "", type="password",
                placeholder=None if is_admin() else "(using organization default)",
            )

        if st.button("▶ download & embed", type="primary"):
            effective_client_id = client_id or default_id
            effective_client_secret = client_secret or default_secret
            if all([site_url, folder_url, effective_client_id, effective_client_secret, collection_name]):
                try:
                    st.info("Authenticating with SharePoint…")
                    sp = SharePointClient.from_credentials(site_url, effective_client_id, effective_client_secret)
                    sp_log = st.empty()
